      ...
      <div className="w-full max-w-6xl mx-auto px-4 py-10 flex flex-col gap-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="flex flex-col gap-4">
            ...
            {isCorporate && (geek?.companyName || geek?.GSTIN) && (
              <div className="bg-white rounded-2xl border border-indigo-100 shadow-sm p-5 flex flex-col gap-3">
                <div className="flex items-center gap-2">
                  <svg className="w-4 h-4 text-indigo-500 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0H3m8-10h.01M12 17h.01M9 17h.01" />
                  </svg>
                  <p className="text-base font-semibold text-gray-900">Company Details</p>
                </div>
                <div className="flex flex-col gap-3 text-sm">
                  {geek?.companyName && (
                    <div className="flex flex-col gap-0.5">
                      <span className="text-xs text-gray-400 uppercase tracking-wide">Company Name</span>
                      <span className="text-gray-800 font-medium">{geek.companyName}</span>
                    </div>
                  )}
                  {geek?.GSTIN && (
                    <div className="flex flex-col gap-0.5">
                      <span className="text-xs text-gray-400 uppercase tracking-wide">GST Number</span>
                      <div className="flex items-center gap-1.5">
                        <span className="text-gray-800 font-mono tracking-wide">
                          {isRequestedService ? geek.GSTIN : `${geek.GSTIN.slice(0, 2)}${'*'.repeat(geek.GSTIN.length - 4)}${geek.GSTIN.slice(-2)}`}
                        </span>
                        <span className="text-xs font-semibold text-green-600 bg-green-50 border border-green-200 px-1.5 py-0.5 rounded-full">✓ Verified</span>
                      </div>
                    </div>
                  )}
                </div>
