...
useEffect(()=>{
    ...
    const hasExpired = request?.createdAt && (new Date(request?.createdAt))?.getTime() + 24 * 60 * 60 * 1000 < Date.now();
    if(hasExpired && request?.status &&  (request?.status === 'Pending' || request?.status === 'Matched')){
    ...
    <div className="grid py-20 grid-cols-12 gap-4 w-full max-w-7xl mx-auto">
                <div className=' md:col-span-8 col-span-12  flex flex-col gap-6 '>
                    <div className='bg-white rounded-lg shadow-md px-4 py-6 max-h-screen overflow-y-scroll custom-scrollbar  w-full flex flex-col gap-6'>
                        ...
                        {request?.video && <div className='w-full flex flex-col gap-4 py-2 items-start'>
                    ...
                    <div className='bg-white rounded-lg shadow-md px-4 py-4 max-h-screen overflow-y-scroll custom-scrollbar  w-full flex flex-col gap-4'>
                        ...
                        {request?.status !== "Completed" && request?.seeker?._id === seeker?._id && <p className='body-2 px-3 text-gray-500'>You can add a rewiew after the service is completed.</p>}
                    ...
                    <div className='w-full bg-white rounded-md py-3 px-4 gap-4 shadow-md flex flex-col'>
                        ...
                        <div className='w-full flex flex-col gap-1 p-3 justify-center items-center bg-gray-200/60 rounded-lg h-36'>
                            <div className='w-14 h-14 overflow-hidden rounded-full relative'>
                                <Image
                                    ...
                                    src={request?.geek?.profileImage?.url ? request?.geek?.profileImage?.url : "/assets/images/placeholder_user.jpg"} alt='Category Image' />
                        ...
                        <div className='flex flex-col w-full gap-4 py-3 border-b border-gray-400'>
                            ...
                            <div className='flex justify-between w-full '>
                               ...
                               {request?.status ==="Completed" || request?.status === "Accepted" ? <span className='text-gray-600'>{request?.geek?.mobile || "N/A"}</span> : <span className='text-gray-600'>+91********</span>}
                            </div>
                        ...
                        <Link href={`/geeks/${request?.geek?._id}`} type='button'   className='text-center w-full cursor-pointer bg-gray-200 text-gray-800 text-sm rounded-md px-3 py-2'>
                            View Profile
                        </Link>
                    </div>  
